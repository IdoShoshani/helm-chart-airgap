...
class HelmPackager:
    ...
    def _ensure_registry_login(self, server: str, username: str, password: str, job_dir: Path):
        """Ensure we're logged into a registry. Uses DOCKER_CONFIG so credentials are stored
        in the app's config (same location crane and helm pull use)."""
        try:
            login_cmd = [
    ...
    def _run_command(self, cmd: List[str], job_dir: Path, check: bool = True) -> subprocess.CompletedProcess:
        ...
        start = datetime.now()
        # Use DOCKER_CONFIG for helm and crane so they read credentials from app's config
        use_auth_env = cmd and (
            cmd[0] == self.crane_bin or self.crane_bin in str(cmd[0]) or
            cmd[0] == self.helm_bin or self.helm_bin in str(cmd[0])
        )
        run_env = self._crane_env() if use_auth_env else None
        try:
...
"""
    ...
    def _build_bundle(self, job_dir: Path, chart_file: Path, chart_name: str, chart_version: str, 
        ...
        # Generate bundle filename (treat None or string "None" as empty)
        if not bundle_name or (isinstance(bundle_name, str) and bundle_name.strip().lower() == "none"):
        ...
        # Enforce MAX_BUNDLE_SIZE_GB
        max_bytes = settings.MAX_BUNDLE_SIZE_GB * 1024 * 1024 * 1024
        ...
        if bundle_size > max_bytes:
            try:
                ...
                pass
            raise PackagingError(
                f"Bundle size ({bundle_size / (1024**3):.2f} GB) exceeds maximum allowed "
                f"({settings.MAX_BUNDLE_SIZE_GB} GB). Increase MAX_BUNDLE_SIZE_GB in configuration or "
                f"reduce chart/images to fit."
            )
        
        self._log(job_dir, f"Bundle created: {bundle_path} ({bundle_size / (1024**2):.2f} MB)")
        
        return bundle_name
    
    def discover_images_from_repo(self, repo_url: str, chart_name: str, chart_version: Optional[str] = None,
                                  values_yaml: Optional[str] = None, bundle_name: Optional[str] = None,
                                  _job_id: Optional[str] = None, _job_dir: Optional[Path] = None,
                                  manifest_only: bool = False) -> List[str]:
        """Fetch chart, render templates, discover images. Saves images.json and sets status=image_selection.
        When manifest_only=True, only fetches the chart (no render/discover)."""
        ...
        try:
            self._log(job_dir, f"Fetching chart: {chart_name}" + (" (manifest only)" if manifest_only else ""))
            ...
            if manifest_only:
                metadata["status"] = "image_selection"
                metadata["images"] = []
    ...
    def discover_images_from_oci(self, oci_chart: str, chart_version: Optional[str] = None,
                                values_yaml: Optional[str] = None, bundle_name: Optional[str] = None,
                                _job_id: Optional[str] = None, _job_dir: Optional[Path] = None,
                                manifest_only: bool = False) -> List[str]:
        """Fetch OCI chart, render templates, discover images. Saves images.json and sets status=image_selection.
        When manifest_only=True, only fetches the chart (no render/discover)."""
        if _job_id is not None and _job_dir is not None:
            job_id, job_dir = _job_id, _job_dir
        else:
        ...
        try:
            ...
            self._log(job_dir, f"Fetching OCI chart: {oci_chart}" + (" (manifest only)" if manifest_only else ""))
    ...
    def package_from_repo(self, repo_url: str, chart_name: str, chart_version: Optional[str] = None,
                         values_yaml: Optional[str] = None, bundle_name: Optional[str] = None,
                         include_images: bool = True, image_list_override: Optional[List[str]] = None,
                         _job_id: Optional[str] = None, _job_dir: Optional[Path] = None,
                         _resume_from_discovery: bool = False, _resume_bundle_name: Optional[str] = None) -> str:
        ...
        try:
            ...
            if _resume_from_discovery:
                # Resume: chart already fetched, use image_list_override for download + build
                job_json_path = job_dir / "job.json"
                if not job_json_path.exists():
                    raise PackagingError("Job metadata not found for resume")
                with open(job_json_path, "r") as f:
                    metadata = json.load(f)
                chart_name = metadata.get("chart_name", "chart")
                chart_version = metadata.get("chart_version", "latest")
                chart_files = list(job_dir.glob("*.tgz"))
                if not chart_files:
                    raise PackagingError("Chart file not found in job dir")
                chart_file = chart_files[0]
                images = image_list_override or []
                metadata["status"] = "in_progress"
                metadata["images"] = images
                metadata["images_count"] = len(images)
                if _resume_bundle_name is not None:
                    metadata["bundle_name"] = _resume_bundle_name
                self._save_job_metadata(job_dir, metadata)
                bundle_name = _resume_bundle_name or metadata.get("bundle_name")
            else:
                self._write_progress(job_dir, "fetch_chart", 0, "Fetching chart from repository", {"chart": chart_name})
                ...
                # Step 4: Discover images (or use override)
                if self._check_cancelled(job_dir):
                    raise JobCancelledError("Job was cancelled by user")
                self._write_progress(job_dir, "discover_images", 32, "Discovering container images")
                if image_list_override is not None:
                    images = image_list_override
                else:
                    images = self._discover_images(render_dir, job_dir)
                metadata["images"] = images
                metadata["images_count"] = len(images)
    ...
    def package_from_oci(self, oci_chart: str, chart_version: Optional[str] = None,
        ...
        try:
            ...
            if _resume_from_discovery:
                ...
                chart_name = metadata.get("chart_name", "chart")
                chart_version = metadata.get("chart_version", "latest")
                oci_chart = metadata.get("oci_chart", oci_chart)
                chart_files = list(job_dir.glob("*.tgz"))
                if not chart_files:
                    raise PackagingError("Chart file not found in job dir")
                chart_file = chart_files[0]
